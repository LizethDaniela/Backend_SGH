...
class AmbienteController {
  ...
  public async create(request: Request, response: Response) {
    ...
    const resultambiente = await this.ambienteRepository.create({ ambiente, capacidad, piso, tipo, disponibilidad_tiempo });
    response.status(201).json({ ambienteResponse: resultambiente });
  }

  public async update(request: Request, response: Response) {
    const { id } = request.params;
    const { ambiente, capacidad, piso, tipo, disponibilidad_tiempo }: IAmbiente = request.body;
    const resultambiente = await this.ambienteRepository.update(id, { ambiente, capacidad, piso, tipo, disponibilidad_tiempo });
    response.status(201).json({ ambienteResponse: resultambiente });
  }
